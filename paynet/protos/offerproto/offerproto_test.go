...
func TestFiveNodes(t *testing.T) {
	...
	// Node 1
	trans1, _, cProto1, rProto1, shutdown1, err := getNode(ctx, 0)
	assert.Nil(t, err)
	defer shutdown1()

	// Node 2
	trans2, _, cProto2, rProto2, shutdown2, err := getNode(ctx, 1)
	assert.Nil(t, err)
	defer shutdown2()

	// Node 3
	trans3, _, cProto3, rProto3, shutdown3, err := getNode(ctx, 2)
	assert.Nil(t, err)
	defer shutdown3()

	// Node 4
	_, _, _, rProto4, shutdown4, err := getNode(ctx, 3)
	assert.Nil(t, err)
	defer shutdown4()

	// Node 5
	trans5, _, cProto5, rProto5, shutdown5, err := getNode(ctx, 4)
	assert.Nil(t, err)
	defer shutdown5()
	...
	routeOut, errOut := rProto1.rs.ListRoutesTo(ctx, crypto.FIL, addr3)
	...
	routeOut, errOut = rProto1.rs.ListRoutesTo(ctx, crypto.FIL, addr4)
	offers = make([]fcroffer.PayOffer, 0)
	for route := range routeOut {
		offer, err := rProto1.QueryOffer(ctx, crypto.FIL, route, big.NewInt(1234))
		assert.Nil(t, err)
	...
	routeOut, errOut = rProto1.rs.ListRoutesTo(ctx, crypto.FIL, addr5)
	offers = make([]fcroffer.PayOffer, 0)
	for route := range routeOut {
		offer, err := rProto1.QueryOffer(ctx, crypto.FIL, route, big.NewInt(1234))
		assert.Nil(t, err)
...
func getNode(ctx context.Context, index int) (*trans.TransactorImpl, *addrproto.AddrProtocol, *paychproto.PaychProtocol, *OfferProtocol, func(), error) {
	// New signer
	...
	oProto := NewOfferProtocol(h, aProto, signer, peerMgr, offerMgr, pservMgr, rs, pam, Opts{})
	shutdown := func() {
		oProto.Shutdown()
		cProto.Shutdown()
		aProto.Shutdown()
	...
	return transactor, aProto, cProto, oProto, shutdown, nil
}
